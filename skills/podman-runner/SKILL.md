---
name: podman-runner
description: Use when the user wants to run an experiment script, command, or pipeline step inside a reproducible container — phrases like "run this in a container", "containerize this run", "execute train.py under podman", "reproduce this run". Handles the Containerfile, build, GPU passthrough, log capture, and `runs/<id>/` provenance. Do not invoke for local-only dev loops or for production deployments.
---

# podman-runner

Single entry point for executing experiment commands inside a
[podman](https://podman.io) container. The skill guarantees the
command runs in a reproducible environment, captures logs, and writes
a provenance-complete `runs/<id>/` directory matching the layout in
`experiment-structure`.

## When to use

- The user asks to *run* a script or command and reproducibility
  matters: "train the model", "run the sweep", "reproduce
  run 2026-03-10-lr-sweep".
- A new experiment has just been scaffolded (`experiment-structure`)
  and the `Containerfile` is empty.
- CI or a teammate will need to run the same command on another
  machine.

Skip this skill for:
- quick interactive debugging loops (just run the script directly),
- deploying a service (use a proper orchestration tool),
- commands that don't produce artifacts worth preserving (e.g.
  `ruff check`).

## Prerequisites (assumed, not installed by this skill)
...
configs/base.yaml`, this skill:

1. Builds (or rebuilds, if Containerfile changed) an image tagged
   `<project>:<short-git-hash>`.
2. Generates a run id (`YYYY-MM-DD-HHMM-<slug>`) and creates
   `runs/<id>/`.
3. Writes provenance files into the run directory **before**
...
5. On success: writes a small `runs/<id>/README.md` stub ("Started
   at …, finished at …, command: …, exit code: 0").
6. On failure: does **not** delete the run directory — the partial
...
This skill wraps that with provenance capture. Pseudocode for the
