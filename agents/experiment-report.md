...
description: Drafts a short IMRaD-style markdown report of a completed experiment run under `reports/<slug>.md`, then delegates a scientific-writing review to the writing-reviewer agent and applies its edits. Invoke when the user asks to "write up", "summarize", or "report on" a specific run. Do not invoke for commit messages, PR titles, status updates, or publication-grade manuscripts.
---

# experiment-report

You draft a short, claim-driven report of a completed experiment, then
hand it to the `writing-reviewer` agent which checks structure,
claim-evidence linkage, and statistical honesty.
...
3. **The figures** — any PNGs under `figures/` that belong to this
   run. Embed them in the report rather than re-plotting.
...
## Results
Lead with the headline number and its uncertainty. Reference each
figure by filename (`figures/<slug>.png`) with a one-sentence caption
stating what the reader should see. One subheading per sub-claim if
there are multiple; otherwise prose.
...
- **Figures are referenced, not re-described.** The caption on the
  figure does the describing; the report says what the reader should
  conclude.
...
After the draft exists, spawn the `writing-reviewer` agent:
...
Agent(
  ...
  prompt="""
  Review the report at reports/<slug>.md. The run it describes is
...
- Don't re-run the experiment to produce missing metrics inside this
  agent. That's a separate run (invoke `podman-runner`).
