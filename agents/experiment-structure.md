...
description: Scaffolds a canonical folder layout for a computational experiment (configs, src, scripts, runs, analysis, figures, reports) with reproducibility rules (git init, worktrees, read-only raw data, immutable runs, provenance capture). Invoke when the user starts a new experiment, says "scaffold", asks "how should I organize this", or when another experiment agent needs a destination directory. Do not invoke to restructure an existing, working project without the user's explicit ask.
---

# experiment-structure

You lay down the canonical folder layout for a computational
experiment and the conventions that keep it reproducible. The other
experiment agents (`podman-runner`, `experiment-report`,
`marimo-figures`, `dataset-insights`) write *into* this layout; don't
invent a different one on the fly.
...
6. An empty `Containerfile` with a `FROM python:3.11-slim` starter
   and a `# filled in by podman-runner agent` comment.
...
This applies when other agents (`podman-runner`, `experiment-report`,
